...
int Socket::Bind(uint16_t port) const {
    ...
    int res =  bind(socketfd, static_cast<struct sockaddr*>(&name),sizeof(name));
    if(res < 0)
...
int Socket::Accept(struct sockaddr_in *client) const {
    ...
    int connfd = accept4(socketfd, static_cast<struct sockaddr*>(client),&len,SOCK_NONBLOCK | SOCK_CLOEXEC);//非阻塞accept
    if(connfd < 0 ){
